...
describe( 'unit:queueing', () => {
	...
	describe( 'getBatchedMessages', () => {
		...
		it( 'batches messages (same provider & dryRun status)', () => {
			const tokens = [ 'a', 'b', 'c' ];
			const queuedMessages = tokens.map( ( token ) => new SingleDeviceMessage( token, PushProvider.APNS, MessageType.CheckEchoV1, {},
				false ) );
		...
		it( 'respects max batch size', () => {
			const queuedMessages = [];
			const randomToken = () => Math.random().toString( 36 ).replace( /[^a-z]+/g, '' ).slice( 0, 10 );
