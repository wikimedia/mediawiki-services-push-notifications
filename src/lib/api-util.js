 ...
 *
 * @param {!express.Application} app the incoming request object
...
function mwApiPost( app, query, headers = {} ) {
	...
	return preq( request ).then( ( response ) => {
		if ( app.cookies && response.headers[ 'set-cookie' ] ) {
			...
			if ( cookieDomain ) {
				const cookie = response.headers[ 'set-cookie' ].map( ( c ) => app.cookies.jar.setCookie( c, cookieDomain ) );
...
function mwApiGetToken( app, type = TokenType.CSRF ) {
	return mwApiPost( app, { action: 'query', format: 'json', meta: 'tokens', type } )
		.then( ( rsp ) => rsp && rsp.body && rsp.body.query && rsp.body.query.tokens &&
...
function mwApiLogin( app ) {
	if (
		!app.conf.mw_subscription_manager_username ||
		!app.conf.mw_subscription_manager_password
	) {
